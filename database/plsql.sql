...
DELIMITER / / CREATE PROCEDURE sp_CreateMegacorporation (
    IN m_name VARCHAR(60),
    IN m_industry VARCHAR(60),
    OUT m_id INT
) BEGIN
INSERT INTO
    Megacorporations (name, industry)
VALUES
    (m_name, m_industry);

-- Store the ID of the last inserted row
SELECT
    LAST_INSERT_ID () into m_id;
...
DELIMITER / / CREATE PROCEDURE sp_UpdateMegacorporation (
    IN m_id INT,
    IN m_name VARCHAR(60),
    IN m_industry VARCHAR(60)
) BEGIN
...
DELIMITER / / CREATE PROCEDURE sp_DeleteMegacorporation (IN m_id INT) BEGIN DECLARE error_message VARCHAR(255);
...
WHERE
    Megacorporations_megacorp_id = m_id;
...
set
    error_message = CONCAT (
        'No matching record found in Megacorporations for id: ',
        m_id
    );
...
DELIMITER / / CREATE PROCEDURE sp_CreateBreach (
...
DELIMITER / / CREATE PROCEDURE sp_UpdateBreach (
    IN b_id INT,
    IN b_date_of_breach DATETIME,
    IN b_ongoing TINYINT (4),
    IN b_hack_type VARCHAR(60),
...
UPDATE Breaches
SET
    date_of_breach = b_date_of_breach,
    ongoing = b_ongoing,
...
DELIMITER / / CREATE PROCEDURE sp_DeleteBreach (IN b_id INT) BEGIN DECLARE error_message VARCHAR(255);
...
set
    error_message = CONCAT (
        'No matching record found in Breaches for id: ',
        b_id
    );
...
DELIMITER / / CREATE PROCEDURE sp_CreateCyberAgent (
...
VALUES
    (c_name, c_job_title, c_hired_by_us, c_location_fk);

-- Store the ID of the last inserted row
SELECT
    LAST_INSERT_ID () INTO c_id;
...
DELIMITER / / CREATE PROCEDURE sp_UpdateCyberAgent (
    IN c_id INT (11),
    IN c_name VARCHAR(60),
    IN c_job_title VARCHAR(60),
    IN c_hired_by_us TINYINT (4),
...
DELIMITER / / CREATE PROCEDURE sp_DeleteCyberAgent (IN c_id INT) BEGIN DECLARE error_message VARCHAR(255);
...
SET
    error_message = CONCAT (
        'No matching record found in CyberAgents for id: ',
        c_id
    );
...
DELIMITER / / CREATE PROCEDURE sp_CreateAsset (
...
INSERT INTO
    Assets (name, asset_type, status, value)
VALUES
    (a_name, a_asset_type, a_status, a_value);

-- Store the ID of the last inserted row
SELECT
    LAST_INSERT_ID () into a_id;
...
DELIMITER / / CREATE PROCEDURE sp_UpdateAsset (
    IN a_id INT,
    IN a_name VARCHAR(60),
    IN a_asset_type VARCHAR(60),
    IN a_status VARCHAR(60),
...
DELIMITER / / CREATE PROCEDURE sp_DeleteAsset (IN a_id INT) BEGIN DECLARE error_message VARCHAR(255);
...
SET
    error_message = CONCAT (
        'No matching record found in Assets for id: ',
        a_id
    );
...
DELIMITER / / CREATE PROCEDURE sp_CreateLocation (
...
INSERT INTO
    Locations (lat, `long`, name, building_type)
VALUES
    (l_lat, l_long, l_name, l_building_type);

-- Store the ID of the last inserted row
SELECT
    LAST_INSERT_ID () into l_id;
...
DELIMITER / / CREATE PROCEDURE sp_UpdateLocation (
    IN l_id INT,
    IN l_lat DECIMAL(10, 8),
    IN l_long DECIMAL(11, 8),
    IN l_name VARCHAR(60),
...
DELIMITER / / CREATE PROCEDURE sp_DeleteLocation (IN l_id INT) BEGIN DECLARE error_message VARCHAR(255);
...
SET
    error_message = CONCAT (
        'No matching record found in Locations for id: ',
        l_id
    );
...
DELIMITER / / CREATE PROCEDURE sp_CreateMegacorporationsHasLocations (IN m_megacorp_id INT, IN l_location_id INT) BEGIN DECLARE error_message VARCHAR(255);
...
IF EXISTS (
    ...
    WHERE
        Megacorporations_megacorp_id = m_megacorp_id
        AND Locations_location_id = l_location_id
) THEN
SET
    error_message = CONCAT (
        'This megacorporation/location relationship already exists.'
    );
...
DELIMITER / / CREATE PROCEDURE sp_UpdateMegacorporationsHasLocations (
...
WHERE
    Megacorporations_megacorp_id = old_megacorp_id
    AND Locations_location_id = old_location_id;
...
DELIMITER / / CREATE PROCEDURE sp_DeleteMegacorporationsHasLocations (IN m_megacorp_id INT, IN l_location_id INT) BEGIN DECLARE error_message VARCHAR(255);
...
set
    error_message = CONCAT (
        'No matching record found in MegacorporationsHasLocations for Megacorporation ID: ',
        m_megacorp_id,
        ' and Location ID: ',
        l_location_id
    );
...
DELIMITER / / CREATE PROCEDURE sp_CreateBreachesHasCyberAgents (IN b_breach_id INT, IN b_agent_id INT) BEGIN DECLARE error_message VARCHAR(255);
...
IF EXISTS (
    ...
    FROM
        BreachesHasCyberAgents
    WHERE
        Breaches_breach_id = b_breach_id
        AND CyberAgents_agent_id = b_agent_id
) THEN
SET
    error_message = 'This breach/cyberagent relationship already exists.';
...
INSERT INTO
    BreachesHasCyberAgents (Breaches_breach_id, CyberAgents_agent_id)
VALUES
    (b_breach_id, b_agent_id);
...
DELIMITER / / CREATE PROCEDURE sp_UpdateBreachesHasCyberAgents (
...
WHERE
    Breaches_breach_id = old_breach_id
...
DELIMITER / / CREATE PROCEDURE sp_DeleteBreachesHasCyberAgents (IN b_breach_id INT, IN c_agent_id INT) BEGIN DECLARE error_message VARCHAR(255);
...
DELIMITER / / CREATE PROCEDURE sp_CreateAssetsHasBreaches (IN a_asset_id INT, IN b_breach_id INT) BEGIN DECLARE error_message VARCHAR(255);
...
IF EXISTS (
    ...
    FROM
        AssetsHasBreaches
    WHERE
        Assets_asset_id = a_asset_id
        AND Breaches_breach_id = b_breach_id
) THEN
SET
    error_message = 'This asset/breach relationship already exists.';
...
INSERT INTO
    AssetsHasBreaches (Assets_asset_id, Breaches_breach_id)
VALUES
    (a_asset_id, b_breach_id);
...
DELIMITER / / CREATE PROCEDURE sp_UpdateAssetsHasBreaches (
    IN old_asset_id INT,
    IN old_breach_id INT,
    IN new_asset_id INT,
    IN new_breach_id INT
...
UPDATE AssetsHasBreaches
SET
    Assets_asset_id = new_asset_id,
    Breaches_breach_id = new_breach_id
WHERE
    Assets_asset_id = old_asset_id
...
DELIMITER / / CREATE PROCEDURE sp_DeleteAssetsHasBreaches (IN a_asset_id INT, IN b_breach_id INT) BEGIN DECLARE error_message VARCHAR(255);
